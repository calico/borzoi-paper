...
def visualize_input_gradient_pair(att_grad_wt, att_grad_mut, fig_size=(8, 1), plot_start=0, plot_end=100, label_1='WT', label_2='Mut', save_figs=False, fig_name='') :
...
def plot_coverage_tracks(
    ...
    for track_i, [track_name, track_color, track_label] in enumerate(zip(track_names, track_colors, track_labels)) :
        ...
        if highlight_bin is not None :
            l1 = plt.plot([highlight_bin, highlight_bin], [0., max_y_i * 0.9995], color='black', linewidth=0.5, linestyle='--', alpha=0.5, zorder=10, label='Highlight')
        ...
        if gene_slice is not None :
            if plot_other_genes :
                plt.ylim(global_y_min + (-0.25 - isoform_height_frac) * max_y_i, max_y_i)
            else :
                plt.ylim(global_y_min + (-0.15 - isoform_height_frac) * max_y_i, max_y_i)
        else :
            plt.ylim(global_y_min, max_y_i)
        ...
        for target_i in range(n_targets):
            ...
            if transform_old:
                ...
                if scales is None :
                    ...
                    seq_cov = scales[target_i] * seq_cov
            else:
                ...
                if scales is None :
                    ...
                    seq_cov_nt = scales[target_i] * seq_cov_nt
