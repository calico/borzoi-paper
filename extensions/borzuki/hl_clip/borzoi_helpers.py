...
def get_prediction_gradient(
    ...
    pred_grads = np.zeros((len(sequence_one_hots), len(fold_index), 393216, 4))
    ...
    for fold_i, fold_ix in enumerate(fold_index) :
        ...
        input_sequence = tf.keras.layers.Input(shape=(393216, 4), name="sequence")
...
def get_ism(
    ...
    pred_ism = np.zeros((len(sequence_one_hots), len(models), 393216, 4))
...
def get_ism_shuffle(
    ...
    pred_shuffle = np.zeros((len(sequence_one_hots), len(models), 393216, n_samples))
    pred_ism = np.zeros((len(sequence_one_hots), len(models), 393216, 4))
...
def plot_coverage_tracks(
    ...
    pair_order=[0, 1],
    plot_start_rel=512,
    plot_end_rel=393216-512,
    normalize_start_rel=512,
    normalize_end_rel=393216-512,
    ...
    #Pool replicate tracks and apply a custom normalizing scale
    y_1_pooled = []
    y_2_pooled = []
    for track_i, track_index in enumerate(track_indices) :
        y_1_pooled.append(np.mean(y_1[..., track_index], axis=(0, 1, 3))[:, None] / (norm_scales[track_i] if norm_scales is not None else 1.))
        y_2_pooled.append(np.mean(y_2[..., track_index], axis=(0, 1, 3))[:, None] / (norm_scales[track_i] if norm_scales is not None else 1.))
